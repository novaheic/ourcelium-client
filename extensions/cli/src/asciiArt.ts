...
export const OURCELIUM_ASCII_ART = `
${mind.multiline(` ██████╗ ██╗   ██╗██████╗  ██████╗███████╗██╗     ██╗██╗   ██╗███╗   ███╗
██╔═══██╗██║   ██║██╔══██╗██╔════╝██╔════╝██║     ██║██║   ██║████╗ ████║
██║   ██║██║   ██║██████╔╝██║     █████╗  ██║     ██║██║   ██║██╔████╔██║
██║   ██║██║   ██║██╔══██╗██║     ██╔══╝  ██║     ██║██║   ██║██║╚██╔╝██║
...
// ASCII art for just "OU" (first two letters), shown on narrow terminals
const OU_ASCII_ART = `
${mind.multiline(`  ██████╗ ██╗   ██╗
 ██╔═══██╗██║   ██║
 ██║   ██║██║   ██║
...
export function getDisplayableAsciiArt(): string {
  ...
  if (terminalWidth >= MIN_WIDTH_FOR_ASCII_ART) {
    return OURCELIUM_ASCII_ART;
  }

  // If terminal is too narrow, show just "OU" ASCII art
  return OU_ASCII_ART;
}
