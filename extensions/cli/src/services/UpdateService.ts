...
export class UpdateService extends BaseService<UpdateServiceState> {
  ...
  private async checkAndAutoUpdate() {
    ...
    try {
      ...
      if (
        ...
        this.setState({
          ...
          message: isUpdateAvailable
            ...
            : `Ourcelium CLI v${this.currentState.currentVersion}`,
          isUpdateAvailable,
          latestVersion,
        });
      }
      ...
      this.setState({
        ...
        message: `Ourcelium CLI v${this.currentState.currentVersion}`,
        error,
      });
    }
  }
  ...
  async performUpdate(isAutoUpdate?: boolean) {
    ...
    try {
      ...
      // Install the update
      const { stdout, stderr } = await execAsync("npm i -g @ourcelium/cli");
